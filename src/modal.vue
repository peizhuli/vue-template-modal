<template>
  <transition name="modal" mode="out-in">
    <div class="t-modal-container" ref="modalContainer" v-if="options">
      <div class="t-modal-mark" v-if="options.hasMark"></div>
      <div class="t-modal-content" :style="modelStyle">
        <button class="t-modal-btn" v-if="options.modelType != 1 && options.hasCloseButton" :style="{background: options.activeBgColor, color: options.activeColor}" @click="closeModal">×</button>
        <div class="t-modal-header" v-if="options.modelType != 1 && options.hasHeader" :style="{background: options.activeBgColor, color: options.activeColor}">
          <span>{{ options.headerText }}</span>
        </div>
        <div class="t-modal-body">
          <div ref="bodyTemplate" v-html="options.bodyHtml"></div>
        </div>
        <div class="t-modal-footer" v-if="options.modelType != 1 && options.hasFooterButtons">
